...
//					, {extend: true}
	...
	, replaceNewExpression: function(node) {
		...
		var isMemberExpression = node.callee.type === "MemberExpression";
		var isFunctionExpression = node.callee.type === "FunctionExpression";
		var args = node["arguments"];
		var argsLength = args.length;
		var isSuper = isMemberExpression
			? node.callee.object.$originalName === 'super' || node.callee.object.name === 'super'
		...
		var tempVarInstance = core.getScopeTempVar(node, node.$scope);
		var tempVar = core.getScopeTempVar(node, node.$scope);
				...
				if ( core.is.isArrayExpression(argument) ) {
					...
					if( isFirst ) {
						...
						if( lastNotNullElementIndex === -1 ) {// [ , , ...spread] case
							...
							var lastElement = elements[lastNotNullElementIndex];
							var from$0 = getRange(lastElement)[1]
								, to = argumentRange[0]
							;
							...
							$that$0.alter.replace(
								from$0
								, to
								, $that$0.alter.get(from$0, to) + '|' + concatStr + arrayHolesString$0 + callIteratorFunctionName + "("
